...
void main() {
  ...
  test('can retrieve schedule for a stop and day type', () async {
    ...
    await db.operatorsDao.insertOperator(
      Operator(id: 'op1', name: 'Op 1', transportMode: TransportMode.tram),
    );
    await db.stopsDao.insertStop(
      Stop(
        id: 's1',
        name: 'Stop 1',
        lat: 36.5,
    ...
    await db.tripsDao.insertTrip(
      Trip(id: 't1', routeId: 'r1', direction: Direction.outbound),
    );

    await db.stopTimesDao.insertStopTime(
      StopTime(
        tripId: 't1',
        stopId: 's1',
        sequence: 1,
    ...
    final schedule = await db.stopTimesDao.getStopTimesByStopAndDay(
      's1',
      DayType.weekday,
    );
    ...
    final emptySchedule = await db.stopTimesDao.getStopTimesByStopAndDay(
      's1',
      DayType.saturday,
    );
